...
print(head(sort(pr.out$x[,1])))

## Far left (very liberal)
print(head(sort(pr.out$x[,1], decreasing=TRUE)))

# PC 2
print(head(sort(pr.out$x[,2])))
...
set.seed (1234567)
print('With nstart = 1')
km.out = kmeans (votes,5, nstart = 1)
km.out$tot.withinss
...
km.out =kmeans (votes,5, nstart = 100)
km.out$tot.withinss
